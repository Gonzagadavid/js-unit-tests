...
/*
  ...
  - Uma chave `pay` que, quando chamada, invoca uma função que soma o valor de todos os pedidos e dá o preço
   com acréscimo de 10%.

  IMPORTANTE: FAÇA OS TESTES E PASSOS DE ACORDO COM A ORDEM INDICADA!

  OBS: Lembre-se que você não precisa se preocupar com o describe e o it por enquanto, isso será aprendido
   posteriormente.
*/
...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const objetoRetornado = createMenu({ food: { pastel: 6.45 }, drink: { caldoDeCana: 3.5 } });
    // objetoRetornado.fetchMenu() // Retorno: objetoQualquer

    assert.deepStrictEqual(objetoRetornado.fetchMenu(), {
      food: { pastel: 6.45 },
      drink: { caldoDeCana: 3.5 },
    });
    ...
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi
    // pedido, conforme registrado em `objetoRetornado.consumption`
    const objetoRetornado2 = createMenu({ food: { coxinha: 3.5 }, drink: { agua: 3 } });
    objetoRetornado2.order('coxinha');
    objetoRetornado2.order('agua');
    objetoRetornado2.order('coxinha');
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos

    assert.strictEqual(objetoRetornado2.pay(), 10);
